...
export default function Game() {
    ...
    const pathMinistre = "images/ministre-bg.png"
        ...
        <>
            ...
            <div className={'max-h-[calc(100vh-60px)] overflow-y-hidden max-md:hidden'}>
                ...
                <div className="w-full h-screen flex items-center justify-center">
                    <Sprite id={"background"} path={BackgroundIMG()} alt={"Un fond d'écran"} height={"full"}
                            width={"full"}/>
                </div>
                {actualDialogue < 6 &&
                    <Personnage id={"president"} path={pathPresident} alt={"Un président important"} position={"right"}
                                talking={setTalkingPresident()}/>}
                {actualDialogue < 6 &&
                    <Personnage id={"ministre"} path={pathMinistre} alt={"Un ministre important"} position={"left"}
                                talking={setTalkingMinistre()}/>}
                {actualDialogue < 6 && <div>
                    {!end && <Discussion dialogues={dialogues[actualDialogue]} actualDiscussion={actualDiscussion}
                                         setActualDiscussion={setActualDiscussion}/>}
                    {actualDiscussion === dialogues[actualDialogue].length - 1 && !end &&
                        <BoutonReponse setSocial={setSocial} setEcolo={setCO2} setEconomie={setEuro} id={actualDialogue}
                                       setResponse={setResponse} setEndOfDiscussion={setEndOfDialogue}
                                       reponseUser={setReponseUser} setDate={setDate}/>}
                </div>}
                {end &&
                    <PopUp
                        choices={reponseUser}
