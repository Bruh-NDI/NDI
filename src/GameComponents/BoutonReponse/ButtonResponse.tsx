...
export function ButtonResponse({ setResponse, setEndOfDiscussion, setEcologie, setSocial, setEconomie, button } : ButtonResponseProps) {
    ...
    return (
        <button id={button.id} onClick={updateValue}  className="transition-transform duration-300 ease-in-out transform bg-amber-50 text-black m-1 rounded-md p-1 border-amber-600 border-2 hover:bg-amber-100 hover:scale-105">{button.text}</button>
    );
